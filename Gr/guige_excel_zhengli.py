...
# print(guige_df)
# guige_1.to_excel(test_file,index=False, header=False)
guige_1.to_excel(save_guige, index=False)


# guige_1_1=guige_1.groupby(['工号', '分箱', '备注'])['作业'].apply(lambda x:x.str.cat(sep=r'')).reset_index()         # 2020-10-19   增加根据分箱号，决定规格
# guige_1_1.to_excel(save_guige, index=False)
...
wuliao_file = gv_Save_Path + r'\model\物料编码.xlsx'
...
df2_no = len(df2)
for i in range(5):      # 5行
    df2.loc[df2_no+i] = ['' for n in range(22)]   # 22列
